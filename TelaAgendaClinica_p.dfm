object TelaAgendaClinica: TTelaAgendaClinica
  Left = 0
  Top = 0
  Caption = 'MRP Com'#233'rcio - Sistema de Gerenciamento Comercial [Agenda]'
  ClientHeight = 354
  ClientWidth = 937
  Color = clGradientActiveCaption
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Visible = True
  WindowState = wsMaximized
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 288
    Width = 937
    Height = 66
    Align = alBottom
    TabOrder = 0
    ExplicitTop = 294
    object Label1: TLabel
      Left = -56
      Top = 5
      Width = 23
      Height = 13
      Caption = 'Data'
    end
    object Label2: TLabel
      Left = 18
      Top = 10
      Width = 87
      Height = 19
      Caption = 'Profissional:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 249
      Top = 10
      Width = 32
      Height = 19
      Caption = 'Data'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object dtpDate: TJvDateTimePicker
      Left = 249
      Top = 30
      Width = 120
      Height = 27
      Date = 41328.866658171300000000
      Format = 'dd/MM/yyyy'
      Time = 41328.866658171300000000
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      DropDownDate = 41328.000000000000000000
    end
    object BitBtnSair: TBitBtn
      Left = 956
      Top = 11
      Width = 86
      Height = 25
      Caption = 'Sair'
      DoubleBuffered = True
      ParentDoubleBuffered = False
      TabOrder = 2
      TabStop = False
      OnClick = BitBtnSairClick
    end
    object DBLookupComboBox1: TDBLookupComboBox
      Left = 18
      Top = 30
      Width = 215
      Height = 27
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      KeyField = 'NOME'
      ListField = 'NOME'
      ListFieldIndex = 1
      ListSource = dsVet
      ParentFont = False
      TabOrder = 1
    end
    object BitBtnPrior: TJvXPButton
      Left = 373
      Top = 29
      Width = 17
      Height = 29
      TabOrder = 3
      Glyph.Data = {
        
        0000000049454E44AE426082}
      OnClick = BitBtnPriorClick
    end
    object BitBtnNext: TJvXPButton
      Left = 392
      Top = 29
      Width = 17
      Height = 29
      Caption = 'JvXPButton1'
      TabOrder = 4
      Glyph.Data = {
        
        E50000000049454E44AE426082}
      OnClick = BitBtnNextClick
    end
    object BitBtnPesquisa: TJvXPButton
      Left = 440
      Top = 24
      Width = 90
      Height = 29
      Caption = 'OK'
      TabOrder = 5
      Glyph.Data = {
        
        6082}
      OnClick = BitBtnPesquisaClick
    end
    object BitBtnImprimeAgenda: TJvXPButton
      Left = 540
      Top = 24
      Width = 90
      Height = 29
      Caption = 'Imprimir'
      TabOrder = 6
      Glyph.Data = {
        
        A7BEBEBE27B9F107F96FA528F96A736D0000000049454E44AE426082}
    end
    object JvXPButton2: TJvXPButton
      Left = 640
      Top = 24
      Width = 90
      Height = 29
      Caption = 'Sair'
      TabOrder = 7
      Glyph.Data = {
        
        CBCECECED3D3D3D4D4D4CECECEDBDBDBFFFFFF000000}
      OnClick = BitBtnSairClick
    end
  end
  object StgAgenda: TJvStringGrid
    Left = 0
    Top = 0
    Width = 937
    Height = 288
    TabStop = False
    Align = alClient
    ColCount = 6
    TabOrder = 1
    OnDblClick = StgAgendaDblClick
    Alignment = taLeftJustify
    FixedFont.Charset = DEFAULT_CHARSET
    FixedFont.Color = clWindowText
    FixedFont.Height = -11
    FixedFont.Name = 'Tahoma'
    FixedFont.Style = []
    ExplicitHeight = 302
    ColWidths = (
      64
      162
      156
      132
      169
      64)
  end
  object dsAgenda: TDataSource
    DataSet = cdsAgenda
    Left = 817
    Top = 57
  end
  object cdsAgenda: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDateTime
        Name = 'data'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'cdveterinario'
        ParamType = ptInput
      end>
    ProviderName = 'dspAgenda'
    Left = 816
    Top = 103
    object cdsAgendaCDAGENDA_VETERINARIA: TIntegerField
      DisplayLabel = 'C'#243'digo'
      FieldName = 'CDAGENDA_VETERINARIA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object cdsAgendaCDVETERINARIO: TIntegerField
      DisplayLabel = 'C'#243'd. Veterianrio'
      FieldName = 'CDVETERINARIO'
      ProviderFlags = [pfInUpdate]
    end
    object cdsAgendaDATA: TDateField
      DisplayLabel = 'Data'
      FieldName = 'DATA'
      ProviderFlags = [pfInUpdate]
      EditMask = '00/00/0000;1;_'
    end
    object cdsAgendaHORA: TTimeField
      FieldName = 'HORA'
      ProviderFlags = [pfInUpdate]
      EditMask = '!90:00;1;_'
    end
    object cdsAgendaCDPROCEDIMENTO: TIntegerField
      DisplayLabel = 'C'#243'd. Procedimento'
      FieldName = 'CDPROCEDIMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object cdsAgendaCDMASCOTE: TIntegerField
      DisplayLabel = 'C'#243'd. Mascote'
      FieldName = 'CDMASCOTE'
      ProviderFlags = [pfInUpdate]
    end
    object cdsAgendaVETERINARIO: TStringField
      DisplayLabel = 'Veterinario(a)'
      FieldName = 'VETERINARIO'
      ProviderFlags = []
      Size = 50
    end
    object cdsAgendaPROCEDIMENTO: TStringField
      DisplayLabel = 'Procedimento'
      FieldName = 'PROCEDIMENTO'
      ProviderFlags = []
      Size = 50
    end
    object cdsAgendaPACIENTE: TStringField
      DisplayLabel = 'Paciente'
      FieldName = 'PACIENTE'
      ProviderFlags = []
      Size = 10
    end
    object cdsAgendaCLIENTE: TStringField
      DisplayLabel = 'Cliente'
      FieldName = 'CLIENTE'
      ProviderFlags = []
      Size = 50
    end
    object cdsAgendaOBSERVACOES: TStringField
      DisplayLabel = 'Observa'#231#245'es'
      FieldName = 'OBSERVACOES'
      ProviderFlags = [pfInUpdate]
      Size = 300
    end
  end
  object qAgenda: TSQLQuery
    MaxBlobSize = 1
    Params = <
      item
        DataType = ftDateTime
        Name = 'data'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'cdveterinario'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'select av.cdagenda_veterinaria,'
      '       av.cdveterinario,'
      '       av.data,'
      '       av.hora,'
      '       av.cdprocedimento,'
      '       av.cdmascote,'
      '       av.observacoes,'
      '       veterinarios.nome as veterinario,'
      '       procedimentos.descricao as procedimento,'
      '       mascote.nome as paciente,'
      '       clientes.apelido as cliente'
      'from agenda_veterinaria av'
      
        'inner join veterinarios on veterinarios.cdveterinario = av.cdvet' +
        'erinario'
      
        'inner join procedimentos on procedimentos.cdprocedimento = av.cd' +
        'procedimento'
      'inner join mascote on mascote.cdmascote = av.cdmascote'
      'inner join clientes on clientes.cdcliente = mascote.cdcliente'
      'where av.data =:data'
      'and av.cdveterinario=:cdveterinario')
    SQLConnection = DMPrincipal.SQLCon
    Left = 813
    Top = 205
    object qAgendaCDAGENDA_VETERINARIA: TIntegerField
      FieldName = 'CDAGENDA_VETERINARIA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qAgendaCDVETERINARIO: TIntegerField
      FieldName = 'CDVETERINARIO'
      ProviderFlags = [pfInUpdate]
    end
    object qAgendaDATA: TDateField
      FieldName = 'DATA'
      ProviderFlags = [pfInUpdate]
    end
    object qAgendaHORA: TTimeField
      FieldName = 'HORA'
      ProviderFlags = [pfInUpdate]
    end
    object qAgendaCDPROCEDIMENTO: TIntegerField
      FieldName = 'CDPROCEDIMENTO'
      ProviderFlags = [pfInUpdate]
    end
    object qAgendaCDMASCOTE: TIntegerField
      FieldName = 'CDMASCOTE'
      ProviderFlags = [pfInUpdate]
    end
    object qAgendaVETERINARIO: TStringField
      FieldName = 'VETERINARIO'
      ProviderFlags = []
      Size = 50
    end
    object qAgendaPROCEDIMENTO: TStringField
      FieldName = 'PROCEDIMENTO'
      ProviderFlags = []
      Size = 50
    end
    object qAgendaPACIENTE: TStringField
      FieldName = 'PACIENTE'
      ProviderFlags = []
      Size = 10
    end
    object qAgendaCLIENTE: TStringField
      FieldName = 'CLIENTE'
      ProviderFlags = []
      Size = 50
    end
    object qAgendaOBSERVACOES: TStringField
      FieldName = 'OBSERVACOES'
      ProviderFlags = [pfInUpdate]
      Size = 300
    end
  end
  object dspAgenda: TDataSetProvider
    DataSet = qAgenda
    Options = [poAllowCommandText, poUseQuoteChar]
    UpdateMode = upWhereKeyOnly
    OnGetTableName = dspAgendaGetTableName
    Left = 818
    Top = 156
  end
  object qVet: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select *'
      'from Veterinarios'
      ''
      '')
    SQLConnection = DMPrincipal.SQLCon
    Left = 875
    Top = 206
  end
  object dspVet: TDataSetProvider
    DataSet = qVet
    UpdateMode = upWhereKeyOnly
    Left = 876
    Top = 150
  end
  object cdsVet: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspVet'
    Left = 880
    Top = 104
    object cdsVetCDVETERINARIO: TIntegerField
      DisplayLabel = 'C'#243'd. Veterin'#225'rio'
      FieldName = 'CDVETERINARIO'
      Required = True
    end
    object cdsVetNOME: TStringField
      DisplayLabel = 'Veterinario'
      FieldName = 'NOME'
      Size = 50
    end
    object cdsVetCRMV: TStringField
      FieldName = 'CRMV'
      Size = 10
    end
  end
  object dsVet: TDataSource
    DataSet = cdsVet
    Left = 881
    Top = 57
  end
end
